...
const Toolbar = () => {
  ...
  const { id } = useParams();
  const {
    activeToolItem, changeToolHandler, undo, redo,
  ...
  return (
    <>
      ...
      <button
        className={cx(classes.aiFloatingBtn, { [classes.aiActive]: showAIPanel })}
        onClick={() => setShowAIPanel((prev) => !prev)}
        title="Open DiagramAI"
      >
        <div className={classes.aiPulseRing} />
        <div className={classes.aiPulseRing2} />
        <span className={classes.aiIcon}>✦</span>
        <span className={classes.aiLabel}>DiagramAI</span>
      </button>
    </>
  );
};
